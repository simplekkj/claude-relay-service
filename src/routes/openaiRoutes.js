...
const OPENAI_CODEX_API_BASE = 'https://chatgpt.com/backend-api/codex'
const OPENAI_CODEX_MODELS_ENDPOINT = `${OPENAI_CODEX_API_BASE}/models`
const OPENAI_CODEX_RESPONSES_ENDPOINT = `${OPENAI_CODEX_API_BASE}/responses`
const OPENAI_CODEX_COMPACT_RESPONSES_ENDPOINT = `${OPENAI_CODEX_API_BASE}/responses/compact`
const OPENAI_CODEX_MEMORIES_TRACE_SUMMARIZE_ENDPOINT = `${OPENAI_CODEX_API_BASE}/memories/trace_summarize`
...
const OPENAI_CODEX_FORWARD_HEADER_KEYS = [
  'version',
  'openai-version',
  'openai-beta',
  'session_id',
...
const OPENAI_UPSTREAM_PASS_THROUGH_HEADER_KEYS = new Set([
  'openai-version',
  'x-request-id',
...
function shouldPassOpenAIUpstreamHeader(lowerKey) {
  return (
    OPENAI_UPSTREAM_PASS_THROUGH_HEADER_KEYS.has(lowerKey) ||
...
function applyOpenAIUpstreamHeaders(res, headers = {}) {
  for (const [key, value] of Object.entries(headers || {})) {
    const lowerKey = key.toLowerCase()
    if (!shouldPassOpenAIUpstreamHeader(lowerKey) || value === undefined) {
      continue
    }
...
function isMemoriesTraceSummarizeRequest(req) {
  return (
    req.path === '/memories/trace_summarize' ||
    req.path === '/v1/memories/trace_summarize' ||
    (typeof req.originalUrl === 'string' &&
      /\/memories\/trace_summarize(?:\?|$)/.test(req.originalUrl))
  )
}

function resolveCodexEndpoint(req) {
  if (isCompactEndpointRequest(req)) {
    return OPENAI_CODEX_COMPACT_RESPONSES_ENDPOINT
  }
  if (isMemoriesTraceSummarizeRequest(req)) {
    return OPENAI_CODEX_MEMORIES_TRACE_SUMMARIZE_ENDPOINT
  }
...
async function fetchCodexModelsFromOpenAIAccount(authContext, req) {
...
async function fetchCodexModelsFromOpenAIResponsesAccount(authContext, req) {
  if (!authContext?.account?.apiKey) {
    const error = new Error('OpenAI-Responses account has no usable apiKey for models fetch')
    error.statusCode = 403
    throw error
  }

  const baseApi = String(authContext?.account?.baseApi || '')
    .trim()
    .replace(/\/+$/, '')
  if (!baseApi) {
    const error = new Error('OpenAI-Responses account baseApi is empty')
  ...
  headers.authorization = `Bearer ${authContext.account.apiKey}`
  headers.accept = 'application/json'
  ...
  const clientVersion =
    typeof req.query?.client_version === 'string' && req.query.client_version.trim()
      ? req.query.client_version.trim()
      : null
  const baseModelsUrl = /\/v1$/i.test(baseApi) ? `${baseApi}/models` : `${baseApi}/v1/models`
  const modelsUrl = clientVersion
    ? `${baseModelsUrl}${baseModelsUrl.includes('?') ? '&' : '?'}client_version=${encodeURIComponent(clientVersion)}`
    : baseModelsUrl
...
async function fetchDynamicCodexModelInfos(authContext, req) {
  if (authContext.accountType === 'openai-responses') {
    return fetchCodexModelsFromOpenAIResponsesAccount(authContext, req)
  }
  return fetchCodexModelsFromOpenAIAccount(authContext, req)
}
...
function buildModelsCatalogUnavailablePayload(accountType, error) {
  ...
  const message = reason
    ? `Failed to load authoritative Codex /models metadata (${accountTypeLabel}): ${reason}`
    : `Failed to load authoritative Codex /models metadata (${accountTypeLabel}).`
  return {
    error: {
      message,
...
const handleResponses = async (req, res) => {
  ...
  try {
    ...
    const isMemoriesRoute = isMemoriesTraceSummarizeRequest(req)

    // 如果不是 Codex CLI 请求，则进行适配
    if (!isCodexCLI && !isMemoriesRoute) {
      // 移除不需要的请求体字段
      ...
      logger.info('📝 Non-Codex CLI memories request detected, forwarding as-is')
    } else {
    ...
    const isStream = isCompactRoute || isMemoriesRoute ? false : req.body?.stream !== false
    const codexEndpoint = resolveCodexEndpoint(req)
    ...
    for (;;) {
      ...
      const headers = {}
      for (const key of OPENAI_CODEX_FORWARD_HEADER_KEYS) {
        if (incoming[key] !== undefined) {
          headers[key] = incoming[key]
        }
      }
...
router.post('/memories/trace_summarize', authenticateApiKey, handleResponses)
router.post('/v1/memories/trace_summarize', authenticateApiKey, handleResponses)
